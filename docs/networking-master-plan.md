...
For the **heating, cooling, and climate design** that depends on this network, see:

- [`heating-master-plan.md`](heating-master-plan.md)
...
- **UDM SE** — main gateway, security appliance, and UniFi controller.
- **UniFi Aggregation Switch (SFP)** — 10G SFP+ core fabric linking the UDM SE, PoE switches, and Unraid.
- **USW Pro Max 48 PoE** — primary PoE distribution to:
  - Raspberry Pi 5 (Home Assistant host).
  - Aqara M3 hub.
  - Aqara M200 hubs (Guest House IR).
  - Olimex ESP32‑POE‑ISO (OpenTherm gateway).
- **USW XG 10 PoE** — uplink switch for high‑throughput access points:
  - U7 Pro XG APs providing WiFi 7 and Thread coverage.
- **Unraid Server (Ryzen 5900X, 100TB)** — runs MariaDB (HA recorder), Frigate, and storage/backup services.

### 2.2 High-Level Topology Diagram
...
[Fiber/Cable WAN]
    └── UDM SE (Gateway & Controller)
          └── [10G SFP+] UniFi Aggregation Switch (SFP)
                ...
                │              └── [PoE]   Olimex ESP32-POE-ISO (OpenTherm)
                ├── [10G SFP+] USW XG 10 PoE
                │              └── [PoE]   U7 Pro XG APs (WiFi7 + Thread)
